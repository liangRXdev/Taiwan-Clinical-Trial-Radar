...
**狀態：規劃中，尚無可用程式碼。** 規格見 [`.ai-review/plan.md`](./.ai-review/plan.md) v0.4（唯一具規範效力的版本），待辦見 [`TODO.md`](./TODO.md)，進度見 [`PROGRESS.md`](./PROGRESS.md)。
...
- **157 組** 原文有差異，其中 **143 組**在忽略空白與全半形後仍然衝突 → `納入條件` 74 組、`台灣預計受試者人數` 21 組（如同日兩筆寫 `19` 與 `31`）、`臨床試驗期別` 3 組（如 `Phase Ⅱ` 對 `Phase Ⅰ,Phase Ⅱ`）
...
前端不載入 166 MB 原始 CSV。payload 採**分層預算**：預設搜尋範圍的冷啟動 ≤1.0 MB gzip（實測基線 715–900 KiB），擴大搜尋範圍與詳情長文字（單列最長 22,490 字元）走按需載入。
