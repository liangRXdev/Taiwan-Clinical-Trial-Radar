...
## 2026-09-22（晚）— trials-index 減肥（schemaVersion 2）

### 結果

**F1：1,452,124 → 1,284,017 bytes，省 168,107（11.6%）。餘裕 3.2% → 14.4%。**

### 先量再決定

`trials-index` 佔 Tier 0 的 98%，所以問題只在它。brotli q5 的邊際成本：

| 組成 | 佔比 |
|---|---:|
| `displayFields` | 38.7% |
| `searchShortLatest` | 31.2% |
| 其餘 12 個小欄位合計 | **2.3%** |

**兩個直覺方向先被數字否決**：

- 縮短 ID、合併布林旗標——12 個小欄位加起來只有 2.3%，brotli 對重複鍵名壓得極好。
  為這個動 ID 穩定性契約不划算。
- `searchShortLatest` 移出——違反 F1 的 readiness 定義（「執行固定查詢並取得正確
  結果集才算就緒」），搬走只是把數字藏起來；且 §2g 已實測拆檔反而更大。

### 採用：純表示法優化

`displayFields` 的值物件改稀疏——`typed` 恰等於 `raw` 時省略、`flags` 為空時省略。
**不改變任何使用者可見行為。** q11 936,731 → 864,722（7.7%），
而實際部署省更多（11.6%）——Cloudflare 的動態壓縮對稀疏結構壓得比 q11 更好。

**最危險的是還原那一行**：`typed` 為 `null` 是有意義的狀態（未提供／無法解析／超界），
寫成 `typed ?? raw` 會把整套 §6.6 sentinel 語意吃掉，而畫面上看不出來。因此：
TS 與 Python 各一個還原函式且**只以「鍵是否存在」判斷**、還原集中在 `densifyIndex`
一處、`tests/web/fixture.ts` 走同一個函式、兩側各有反向哨兵直接斷言 `??`／`or`
會給出不同結果。

### 意外抓到一個會讓月更新永遠不上線的缺陷

schemaVersion bump 讓部署鏈的一個結構缺陷第一次顯形：

`workflow_run.head_sha` 指的是「觸發那個 run 的 commit」，而月更新會**自己再
commit 一個新的**。用它 checkout 會部署資料更新**之前**那份 `public/data/`——
而部署後的線上版本驗證比的是它自己剛建的那份，**所以還會通過**。

> 抓到新資料 → commit 進 repo → 部署舊資料 → 驗證通過 → 綠燈。
> **每個月的資料更新都不會上線，而且完全靜默。**

先前看起來正常是運氣：資料 commit 會被下一次因為別的 push 觸發的 CI 部署順便帶上去。
這次因為前端要 v2、線上資料還是 v1，站台 fail-closed，**F1 量測的 readiness probe
才把它抓出來**——那支腳本接進 gate 才兩小時。

修法：`TARGET_SHA` 依上游分流（CI 用 `head_sha`、月更新退回分支 tip），
並把實際 checkout 的 commit 寫進 log。
