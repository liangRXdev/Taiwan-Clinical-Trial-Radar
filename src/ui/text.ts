...
export const LABEL = {
   ...
   * §7.2：命中來自非最新 cohort。
   *
   * **列出全部日期**（使用者定案 2026-09-22）：一個 Trial 可能有多筆較舊命中、
   * 日期各不相同。只顯示其中一個等於替使用者挑了一筆，而規格在同日衝突那一節
   * 已經定下「不替使用者挑」的原則，這裡沒有理由相反。
   */
  hitFromOlder: (dates: readonly string[]) =>
    `命中來自 ${dates.join("、")} 的審查紀錄`,
  ...
  /**
   * §7.4／E8：`?protocol=` 的值不是計畫書編號（§6.2.2）。
   * **不得說成「查無結果」**——那兩件事對使用者的意義完全不同。
   */
  protocolRejected: (raw: string) =>
    `「${raw}」不是計畫書編號，本站不接受它作為查詢值。請改用搜尋框。`,
  /** §7.4：值的形狀合法但本站沒有這一筆。**查無結果不代表該試驗不存在**（§10）。 */
  protocolNotFound: (raw: string) =>
    `本站沒有計畫書編號「${raw}」的審查紀錄。查無結果不代表該試驗不存在，請改向官方資料來源查詢。`,
} as const;
