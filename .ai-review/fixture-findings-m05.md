...
規格版本：發現時 v0.5；**五個洞已全部併入 v0.6**（見該檔 §15 修訂紀錄），待限縮覆審
...
全部五個已於 **v0.6** 修訂，狀態欄記的是修訂方式；覆審後才算結案。

| # | 嚴重度 | 章節 | 一句話 | v0.6 處置 |
|---|---|---|---|---|
| GAP-8 | **High** | §6.4.2 | 比較鍵表漏了三個語意狀態，同日兩列處於這些狀態時衝突與否無法從規格導出 | 補三列比較鍵 ＋ 窮盡 dispatch（契約 #11） |
| GAP-9 | Medium | §6.6.3 | 「可表示範圍」沒有序位，與「第一個命中者決定結果」字面矛盾 | §6.6.3 拆兩階段；單端超界整個 typed 為 null |
| GAP-10 | Medium | §9.3.5／F1 | `stats.json` 的 facet 名單未封閉，且它在 Tier 0 卻沒計入 F1 的實測基線 | facet 名單封閉為三個（契約 #13）；F1 補檔案清單 |
| GAP-11 | Medium | §9.3.6 | 不變量之間有依賴，但規格沒寫評估順序，使 B6 的「各自獨立反例」寫不出來 | §9.3.6 改為有序 7 條 ＋「無法評估」（契約 #12） |
| GAP-12 | Low | §9.6 | 「以有理數比較」在本資料集規模下**不可驗證**；可驗證的是「不四捨五入」 | §9.6 改寫為「不得四捨五入或截斷」 |
...
| 超界（2^53）＋ 前導零 | §6.6.3 拆兩階段；單端超界整個 typed 為 null | 直接輸出 `9007199254740992` 會失真；`026` 的 raw 不得顯示為 `26` |
