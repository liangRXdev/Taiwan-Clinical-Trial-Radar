...
**schemaVersion 2 起值物件為稀疏**（2026-09-22，M4 後的 payload 減肥）：

| 鍵 | 省略條件 |
|---|---|
| `typed` | **恰等於** `raw`（同型別、同值）時省略；讀取端以 `raw` 還原 |
| `flags` | 為空陣列時省略；讀取端以 `[]` 還原 |

**`typed` 為 `null` 時一律保留。** 那是有意義的狀態（未提供／無法解析／超界），
不是「與 raw 相同」。讀取端**只能以「鍵是否存在」判斷**，寫成 `typed ?? raw`
或 `typed or raw` 會把整套 §6.6 sentinel 語意吃掉，而畫面上看不出來——
使用者會把「來源填了無法解析的東西」讀成「來源就是這個值」。

**為什麼值得做**：`trials-index` 佔 Tier 0 的 98%，而 F1 對真實部署的餘裕只有 3.2%。
實測（brotli q11）936,731 → 864,722，省 7.7%，且**不改變任何使用者可見行為**。
其餘候選都被否決：縮短 ID／合併布林旗標合計只有 2.3%（brotli 對重複鍵名壓得極好）；
`searchShortLatest` 移出違反 F1 的 readiness 定義（「執行固定查詢並取得正確結果集」），
且 §2g 已實測拆檔反而更大。
